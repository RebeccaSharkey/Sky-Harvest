...
public class SCR_PlayerSIloSlotUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerMoveHandler
{
    ...
    private void Awake()
    {
        ...
        UpdateInventorySlot();
        CustomEvents.InventorySystem.PlayerInventory.OnMoveToSilo += MoveToSilo;
    }
    ...
    private void MoveToSilo(InventorySlot invSlot, int siloID)
    {
        if (invSlot == _inventorySlot)
        {
            ...
            CustomEvents.InventorySystem.SiloInventory.OnAddNewItemStack?.Invoke(_inventorySlot.Item, _inventorySlot.Amount, _inventorySlot.Quality, siloID);
        }
    }
    ...
    public void OnMove()
    {
        if(!isSplitting)
        {
            if (!moving)
            {
                ...
                _movingSlot.GetComponent<SCR_MoveableSlot>().SetValues(_inventorySlot, "Player");
                CustomEvents.InventorySystem.SiloInventory.OnOpenInspectionPlayer?.Invoke(_inventorySlot, false, _siloID);
                CustomEvents.ShopSystem.OnOpenInspectionPlayer?.Invoke(_inventorySlot, false, _siloID);
            ...
            else
            {
                ...
                else if((string)CustomEvents.InventorySystem.PlayerInventory.GetMoveablesInventoryOrigin?.Invoke() == "Silo")
                {
    ...
    public void OnMoveClick()
    {
        if(!moving)
        {
            ...
            _toolTip = null;
            CustomEvents.InventorySystem.PlayerInventory.OnRemoveItemStackWithSlot?.Invoke(_inventorySlot);
            CustomEvents.InventorySystem.SiloInventory.OnAddNewItemStack?.Invoke(_inventorySlot.Item, _inventorySlot.Amount, _inventorySlot.Quality, _siloID);
        }
    }
    ...
    public void OnInspect()
    {
        if(!moving)
        {
            ...
            CustomEvents.InventorySystem.SiloInventory.OnOpenInspectionPlayer?.Invoke(_inventorySlot, true, _siloID);
            if (_inventorySlot.Item.ItemType == ItemTypes.crop)
            {
                CustomEvents.InventorySystem.OnShowCropPediaButtons?.Invoke((SCR_CropItems)_inventorySlot.Item, 1);
            }
    ...
    public void SwapOutOfInv()
    {
        siloOptions.SetActive(false);

        gameObject.GetComponentInParent<SCR_SilosInventoryUI>().OnSwapTwoItems(movingInvSlot, _inventorySlot);

        OnMovingStateChange(false);
        CustomEvents.InventorySystem.SiloInventory.OnMovingChanged?.Invoke(false);
    }

    public void MoveToPlayerInv()
    {
        siloOptions.SetActive(false);

        CustomEvents.InventorySystem.SiloInventory.OnRemoveItemStackWithSlot?.Invoke(movingInvSlot, _siloID);
        CustomEvents.InventorySystem.PlayerInventory.OnAddNewItemStack?.Invoke(movingInvSlot.Item, movingInvSlot.Amount, movingInvSlot.Quality);
