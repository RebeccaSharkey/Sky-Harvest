...
public class SCR_SpecialShopUISlot : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerMoveHandler
{
    ...
    public void OnClick()
    {
        ...
        CustomEvents.ShopSystem.SpecialShop.OnOpenInspectionShop?.Invoke(_inventorySlot, false, _shopID);
    ...
    public void OnBuy()
    {
        if (_inventorySlot.Amount > 1)
        {
            ...
            stackOneText.text = string.Format(stackTwo.ToString());
            stackTwoText.text = string.Format(stackOne.ToString());
    ...
    public void OnChangeSellSlider()
    {
        stackOne = _inventorySlot.Amount - (int)splitSlider.value;
        stackTwo = (int)splitSlider.value;
        stackOneText.text = stackTwo.ToString();
        stackTwoText.text = stackOne.ToString();
        switch (_inventorySlot.Quality)
        {
            case ItemQuality.Bad:
                totalPrice.text = string.Format("Total Price: {0}(RD)", _inventorySlot.Item.BuyValueBad * stackTwo);
    ...
    private void BuyStack()
    {
        ...
        if (tempPrice <= tempCurrencyAmount)
        {
            ...
            {
                CustomEvents.InventorySystem.PlayerInventory.OnAddNewItemStack?.Invoke(_inventorySlot.Item, stackOne, _inventorySlot.Quality);
                ...
                if (stackOne >= 1)
                {
                    ...
                    {
                        CustomEvents.ShopSystem.SpecialShop.OnRemoveItemStackWithSlot?.Invoke(_inventorySlot, _shopID);
                    }
            ...
            isSplitting = false;
            splitMenu.SetActive(!splitMenu.activeSelf);
            //Play selling / buying audio

            if (_inventorySlot.Item.ItemType == ItemTypes.unique)
            {
                CustomEvents.Achievements.OnAddToTotalUpgradesBought?.Invoke(1);
            }
    ...
    public void OnInspect()
    {
        ...
        CustomEvents.ShopSystem.SpecialShop.OnOpenInspectionShop?.Invoke(_inventorySlot, true, _shopID);
        if (_inventorySlot.Item.ItemType == ItemTypes.crop)
        {
            CustomEvents.InventorySystem.OnShowCropPediaButtons?.Invoke((SCR_CropItems)_inventorySlot.Item, 2);
        }
